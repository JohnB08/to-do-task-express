...
server.get("/createNewUser", async (req, res)=>{
    ...
    const token = req.headers.id_token as string
    ...
    const createNewUser = await postNewUser(userID as string)
...
server.get("/todoItems", async (req, res)=>{
    ...
    const user_id = await findUser(verifiedToken.user as string)
    if (!user_id.success){
